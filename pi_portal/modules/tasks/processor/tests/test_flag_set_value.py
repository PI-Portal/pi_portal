...
class TestFlagSetValueProcessor:
  ...
  log_message = (
      "DEBUG - {task.id} - {task.type.value} - Processing: '{task}' ...\n"
      "DEBUG - {task.id} - {task.type.value} - Setting flag: "
      "'{task.args.flag_name}' -> '{task.args.value}' .\n"
      "DEBUG - {task.id} - {task.type.value} - Completed: '{task}'!\n"
      "DEBUG - {task.id} - {task.type.value} - Task Timing: '{task}'.\n"
  )
