...
class TestQueueMaintenanceProcessor:
  ...
  log_message_prefix = (
      "DEBUG - {task.id} - {task.type} - None - Processing: '{task}' ...\n"
  )
  log_message_maintenance = (
      "WARNING - {task.id} - {task.type} - {queue} - "
      "Performing maintenance on the '{queue}' task queue ...\n"
  )
  log_message_suffix = (
      "DEBUG - {task.id} - {task.type} - None - Completed: '{task}'!\n"
      "DEBUG - {task.id} - {task.type} - None - Task Timing: '{task}'.\n"
  )
