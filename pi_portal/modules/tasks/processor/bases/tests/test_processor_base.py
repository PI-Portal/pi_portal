...
class TestTaskProcessorBase:
  ...
  def test_log_timings__logging(
    ...
    assert mocked_stream.getvalue() == (
        f"DEBUG - {mocked_task.id} - {mocked_task.type.value} - "
  ...
  def test_process__failure__logging(
    ...
    assert mocked_stream.getvalue() == (
        f"DEBUG - {mocked_task.id} - {mocked_task.type.value} - "
        f"Processing: '{mocked_task}' ...\n"
        f"ERROR - {mocked_task.id} - {mocked_task.type.value} - "
        f"Failed: '{mocked_task}'!\n"
        f"ERROR - {mocked_task.id} - {mocked_task.type.value} - Exception\n" +
  ...
  def test_recover__logging(
    ...
    assert mocked_stream.getvalue() == (
        f"WARNING - {mocked_task.id} - {mocked_task.type.value} - "
